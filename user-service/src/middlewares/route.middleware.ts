...
export const isValidPrivateUrl = (req:Request,res:Response,next:NextFunction) : any => {
    ...
    const originalParams = expressUrl.concat(`/user${currentUrl}`)
    const privateWhitlist : any = Object.values(privateUrl)
    if(privateWhitlist.includes(originalParams)) {
        uberLogger.info(`${currentUrl} is a valid Private Url, Processing the following Url Request`)
        return next()
    }
    return res.status(statusCodes.NOT_FOUND).json({
        message :`${currentUrl} Does not Exists or It is not a valid Url for public`
